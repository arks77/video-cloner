...
#
# Portable: resolves ffmpeg/ffprobe from PATH (override with FFMPEG / FFPROBE env vars).
...
VIDEO="${1:?usage: extract-frames.sh <video_path> <output_dir> [num_frames]}"
OUT_DIR="${2:?usage: extract-frames.sh <video_path> <output_dir> [num_frames]}"
NUM_FRAMES="${3:-12}"
